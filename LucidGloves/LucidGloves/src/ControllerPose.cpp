...
ControllerPose::ControllerPose(vr::ETrackedControllerRole shadowDeviceOfRole, std::string thisDeviceManufacturer) {
	...
	m_controllerPose.qDriverFromHeadRotation.w = 1;
	m_controllerPose.qWorldFromDriverRotation.w = 1;

	//This will be configurable in settings
	m_controllerPose.poseTimeOffset = 0.05f;
};
...
vr::DriverPose_t ControllerPose::UpdatePose() {
	...
	if (trackedDevicePoses[m_shadowControllerId].bPoseIsValid)
	{
		...
		m_controllerPose.vecPosition[0] = trackedDevicePoses[m_shadowControllerId].mDeviceToAbsoluteTracking.m[0][3] + double(vectorOffset.v[0]);
		m_controllerPose.vecPosition[1] = trackedDevicePoses[m_shadowControllerId].mDeviceToAbsoluteTracking.m[1][3] + double(vectorOffset.v[1]);
		m_controllerPose.vecPosition[2] = trackedDevicePoses[m_shadowControllerId].mDeviceToAbsoluteTracking.m[2][3] + double(vectorOffset.v[2]); //- forward
		...
		m_controllerPose.qRotation = MultiplyQuaternion(GetRotation(shadowControllerMatrix), QuaternionFromAngle(1, 0, 0, DegToRad(-90)));
