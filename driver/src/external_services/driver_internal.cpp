...
class DriverInternalServer::Impl {
 public:
  explicit Impl(std::function<crow::response(std::string path, const crow::request& req)> callback) {
    callback_ = callback;
    // set up the routes
    CROW_ROUTE(app_, "/tracking_reference").methods("POST"_method)([&](const crow::request& req) {
      return callback_("tracking_reference", req);
    });

    server_ = app_.port(52050).multithreaded().run_async();
  }
