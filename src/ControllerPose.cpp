...
ControllerPose::ControllerPose(
    ...
    : _poseConfiguration(poseConfiguration), _shadowDeviceOfRole(shadowDeviceOfRole), _thisDeviceManufacturer(std::move(thisDeviceManufacturer)) {
  _calibrationPipe = std::make_unique<NamedPipeListener<CalibrationDataIn>>(
  ...
  _calibrationPipe->StartListening([&](const CalibrationDataIn* data) {
    if (data->start) {
  ...
  if (_poseConfiguration.controllerOverrideEnabled) {
    _shadowControllerId = _poseConfiguration.controllerIdOverride;
  } else {
    _controllerDiscoverer = std::make_unique<ControllerDiscovery>(shadowDeviceOfRole, [&](const ControllerDiscoveryPipeData data) {
      _shadowControllerId = data.controllerId;
      DriverLog("Received controller id from overlay: %i", data.controllerId);
    });

    _controllerDiscoverer->Start();
  }
  _calibration = std::make_unique<Calibration>();
}

ControllerPose::~ControllerPose() {
  _calibrationPipe->StopListening();
  if (_controllerDiscoverer) _controllerDiscoverer->Stop();
}
...
vr::TrackedDevicePose_t ControllerPose::GetControllerPose() const {
  ...
  return trackedDevicePoses[_shadowControllerId];
}

vr::DriverPose_t ControllerPose::UpdatePose() const {
  if (_calibration->IsCalibrating()) return _calibration->GetMaintainPose();

  vr::DriverPose_t newPose = {0};
  newPose.qWorldFromDriverRotation.w = 1;
  newPose.qDriverFromHeadRotation.w = 1;

  if (_shadowControllerId != vr::k_unTrackedDeviceIndexInvalid) {
    if (const vr::TrackedDevicePose_t controllerPose = GetControllerPose(); controllerPose.bPoseIsValid) {
      ...
      const vr::HmdVector3_t vectorOffset = MultiplyMatrix(controllerRotationMatrix, _poseConfiguration.offsetVector);
      ...
      // hand
      newPose.qRotation = MultiplyQuaternion(GetRotation(controllerMatrix), _poseConfiguration.angleOffsetQuaternion);
      ...
      newPose.poseTimeOffset = _poseConfiguration.poseTimeOffset;
    } else {
...
void ControllerPose::StartCalibration(const CalibrationMethod method) const {
  _calibration->StartCalibration(UpdatePose(), method);
}
...
void ControllerPose::CompleteCalibration(const CalibrationMethod method) {
  ...
  _poseConfiguration = _calibration->CompleteCalibration(GetControllerPose(), _poseConfiguration, IsRightHand(), method);
}

void ControllerPose::CancelCalibration(const CalibrationMethod method) const {
  _calibration->CancelCalibration(method);
}

bool ControllerPose::IsCalibrating() const {
  return _calibration->IsCalibrating();
}

bool ControllerPose::IsRightHand() const {
  return _shadowDeviceOfRole == vr::TrackedControllerRole_RightHand;
}
