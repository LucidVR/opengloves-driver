...
std::string LegacyEncodingManager::EncodeForceFeedback(const VRFFBData_t& input) {
  char buffer[50];

  sprintf(buffer, "%d&%d&%d&%d&%d", input.thumbCurl, input.indexCurl, input.middleCurl,
          input.ringCurl, input.pinkyCurl);
