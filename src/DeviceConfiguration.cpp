...
static VREncodingConfiguration GetEncodingConfiguration() {
  switch (static_cast<VREncodingProtocol>(vr::VRSettings()->GetInt32(c_driverSettingsSection, "encoding_protocol"))) {
    case VREncodingProtocol::Legacy: {
      ...
      return {VREncodingProtocol::Legacy, maxAnalogueValue, VRLegacyEncodingConfiguration{}};
    }
    ...
    case VREncodingProtocol::Alpha: {
      ...
      return {VREncodingProtocol::Alpha, maxAnalogueValue, VRAlphaEncodingConfiguration{}};
    }
  }
}

static VRCommunicationConfiguration GetCommunicationConfiguration(const bool isRightHand) {
  ...
  const VREncodingConfiguration encodingConfiguration = GetEncodingConfiguration();
  ...
  switch (static_cast<VRCommunicationProtocol>(vr::VRSettings()->GetInt32(c_driverSettingsSection, "communication_protocol"))) {
    case VRCommunicationProtocol::NamedPipe: {
      ...
      return {VRCommunicationProtocol::NamedPipe, encodingConfiguration, feedbackEnabled, VRCommunicationNamedPipeConfiguration{pipeName}};
    }
    ...
    case VRCommunicationProtocol::BtSerial: {
      ...
      return {VRCommunicationProtocol::BtSerial, encodingConfiguration, feedbackEnabled, VRCommunicationBTSerialConfiguration{name}};
    }
    ...
    case VRCommunicationProtocol::Serial: {
      ...
      return {VRCommunicationProtocol::Serial, encodingConfiguration, feedbackEnabled, VRCommunicationSerialConfiguration{port, baudRate}};
    }
  }
}

static VRPoseConfiguration GetPoseConfiguration(bool isRightHand) {
...
static VRDeviceConfiguration GetDeviceConfiguration(const vr::ETrackedControllerRole& role) {
  const bool isRightHand = role == vr::ETrackedControllerRole::TrackedControllerRole_RightHand;

  vr::CVRSettingHelper settingHelper(vr::VRSettings());

  const VRPoseConfiguration poseConfiguration = GetPoseConfiguration(isRightHand);
  const VRCommunicationConfiguration communicationConfiguration = GetCommunicationConfiguration(isRightHand);
  ...
  switch (static_cast<VRDeviceType>(vr::VRSettings()->GetInt32(c_driverSettingsSection, "device_driver"))) {
    case VRDeviceType::LucidGloves: {
      ...
      return {
          VRDeviceType::LucidGloves,
          serialNumber,
          role,
          poseConfiguration,
          communicationConfiguration,
          VRDeviceLucidglovesConfiguration{serialNumber}};
    }
    ...
    case VRDeviceType::EmulatedKnuckles: {
      ...
      return {
          VRDeviceType::EmulatedKnuckles,
          serialNumber,
          role,
          poseConfiguration,
          communicationConfiguration,
          VRDeviceKnucklesConfiguration{indexCurlAsTrigger, approximateThumb}};
    }
  }
}
...
VRDriverConfiguration GetDriverConfiguration(const vr::ETrackedControllerRole& role) {
  ...
  return {enabled, GetDeviceConfiguration(role)};
}