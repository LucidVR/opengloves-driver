...
vr::EVRInitError DeviceProvider::Init(vr::IVRDriverContext* pDriverContext) {
  ...
  std::shared_ptr<BoneAnimator> boneAnimator =
      std::make_shared<BoneAnimator>(driverPath + "\\resources\\anims\\glove_anim.glb");
  ...
  if (rightConfiguration.enabled) {
    ...
    vr::VRServerDriverHost()->TrackedDeviceAdded(
        m_rightHand->GetSerialNumber().c_str(), vr::TrackedDeviceClass_Controller, m_rightHand.get());
  }
...
std::unique_ptr<DeviceDriver> DeviceProvider::InstantiateDeviceDriver(
    VRDeviceConfiguration_t configuration, std::shared_ptr<BoneAnimator> boneAnimator) {
  ...
  switch (configuration.communicationProtocol) {
    case VRCommunicationProtocol::BTSERIAL: {
      ...
      vr::VRSettings()->GetString(
          c_btserialCommunicationSettingsSection, isRightHand ? "right_name" : "left_name", name, sizeof(name));
    ...
    case VRCommunicationProtocol::SERIAL:
      char port[16];
      vr::VRSettings()->GetString(
          c_serialCommunicationSettingsSection, isRightHand ? "right_port" : "left_port", port, sizeof(port));
  ...
  switch (configuration.deviceDriver) {
    case VRDeviceDriver::EMULATED_KNUCKLES: {
      ...
      vr::VRSettings()->GetString(
          c_knuckleDeviceSettingsSection,
    ...
    case VRDeviceDriver::LUCIDGLOVES: {
      char serialNumber[32];
      vr::VRSettings()->GetString(
          c_lucidGloveDeviceSettingsSection,
          isRightHand ? "right_serial_number" : "left_serial_number",
          serialNumber,
          sizeof(serialNumber));
...
VRDeviceConfiguration_t DeviceProvider::GetDeviceConfiguration(vr::ETrackedControllerRole role) {
  ...
  const auto communicationProtocol =
      (VRCommunicationProtocol)vr::VRSettings()->GetInt32(c_driverSettingsSection, "communication_protocol");
  ...
  const int controllerIdOverride =
      controllerOverrideEnabled
          ? vr::VRSettings()->GetInt32(
                c_poseSettingsSection, isRightHand ? "controller_override_right" : "controller_override_left")
          : -1;
  const bool calibrationButton =
      vr::VRSettings()->GetBool(c_poseSettingsSection, "hardware_calibration_button_enabled");
