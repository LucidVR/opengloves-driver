...
std::unique_ptr<IDeviceDriver> DeviceProvider::InstantiateDeviceDriver(VRDeviceConfiguration_t configuration) {
  ...
  case VRCommunicationProtocol::EXPERIMENTAL_SERIAL: {
    char port[16];
    ...
    VRSerialConfiguration_t serialSettings(port);
      communicationManager = std::make_unique<ASIOSerialCommunicationManager>(serialSettings, std::move(encodingManager));
    break;
  }
