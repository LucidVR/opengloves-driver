...
void DeviceProvider::RunFrame() {
  ...
  while (vr::VRServerDriverHost()->PollNextEvent(&pEvent, sizeof(pEvent))) {
    if (leftHand_ && leftHand_->IsActive() && leftHand_->GetDeviceId() == pEvent.trackedDeviceIndex) {
      leftHand_->OnEvent(pEvent);
    } else if (rightHand_ && rightHand_->IsActive() && rightHand_->GetDeviceId() == pEvent.trackedDeviceIndex) {
      rightHand_->OnEvent(pEvent);
    }
  }
}
