...
DeviceDriver::DeviceDriver(
  ...
  std::copy(
      std::begin(IsRightHand() ? rightOpenPose : leftOpenPose),
      std::end(IsRightHand() ? rightOpenPose : leftOpenPose),
...
vr::EVRInitError DeviceDriver::Activate(uint32_t unObjectId) {
  ...
  m_controllerPose = std::make_unique<ControllerPose>(
      m_configuration.role, std::string(c_deviceDriverManufacturer), m_configuration.poseConfiguration);
